...
void ofApp::update() {
	...
	if (ofGetFrameNum() % 60 < 30) {
		...
		if (ofGetFrameNum() % 60 < 15) {
			...
			r = ofMap(ofGetFrameNum() % 60, 15, 30, R * 0.2, R * 0.6);
		}
	}
	else {
		...
		if (ofGetFrameNum() % 60 < 45) {
			...
			r = ofMap(ofGetFrameNum() % 60, 45, 60, R * 0.6, R * 0.2);
		}
	}
	...
	for (float phi_deg = 0; phi_deg < 360; phi_deg += phi_deg_step) {
		...
		for (float theta_deg = 0; theta_deg < 360; theta_deg += theta_deg_step) {
			...
			auto noise_value = ofNoise(glm::vec4(this->make_point(R, base_r, theta_deg, phi_deg) * 0.009, this->noise_param));
			if (noise_value <= threshold) { continue; }

			auto noise_1 = ofNoise(glm::vec4(this->make_point(R, base_r, theta_deg - theta_deg_step, phi_deg) * 0.009, this->noise_param));
			auto noise_2 = ofNoise(glm::vec4(this->make_point(R, base_r, theta_deg, phi_deg + phi_deg_step) * 0.009, this->noise_param));
			auto noise_3 = ofNoise(glm::vec4(this->make_point(R, base_r, theta_deg, phi_deg - phi_deg_step) * 0.009, this->noise_param));
			auto noise_4 = ofNoise(glm::vec4(this->make_point(R, base_r, theta_deg + theta_deg_step, phi_deg) * 0.009, this->noise_param));
	...
	if (ofGetFrameNum() % 60 < 30) {
		...
		if (ofGetFrameNum() % 60 < 15) {
			...
			r = ofMap(ofGetFrameNum() % 60, 15, 30, R * 0.6, R * 0.2);
		}
	}
	else {
		...
		if (ofGetFrameNum() % 60 < 45) {
			...
			r = ofMap(ofGetFrameNum() % 60, 45, 60, R * 0.2, R * 0.6);
		}
	}
